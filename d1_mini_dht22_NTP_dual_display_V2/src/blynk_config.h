...
/*void dht22() //纯文本显示页面
{
  ...
  html += "<p>MCA地址：" + WiFi.macAddress() + "</p>";
  html += "<p>RSSI：" + String(WiFi.RSSI()) + "db</p>";
  html += "<p>NTP服务器：" + String(ntp_server)  + "db</p>";
  html += "</body></html>";
...
void dht22()
{
  ...
  html += "<h1>Blynk版本DHT22双显示屏小时钟</h1>";
  html += "<h2>Web诊断页面</h2>";
  ...
  html += "</div>";
  html += "<p style=\"line-height: 0;\">温度：" + String(temperature, 1) + "\u00B0C</p>";
  html += "<p style=\"line-height: 0;\">湿度：" + String(humidity, 0) + "%</p>";
  ...
  html += "<p style=\"line-height: 0;\">NTP服务器：" + String(ntp_server) + "</p>";
